[style.css](https://github.com/user-attachments/files/23281553/style.css)
* {
    margin: 0;
    padding: 0;
    box-sizing: border-box;
}

body {
    font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
    min-height: 100vh;
    overflow-x: hidden;
    position: relative;
}

/* Анимированный фон */
.animated-background {
    position: fixed;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    z-index: -1;
    background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%);
}

/* Звезды */
.stars {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    background: transparent;
}

.stars:before {
    content: '';
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    background-image: 
        radial-gradient(2px 2px at 20px 30px, #eee, transparent),
        radial-gradient(2px 2px at 40px 70px, #fff, transparent),
        radial-gradient(1px 1px at 90px 40px, #ddd, transparent),
        radial-gradient(1px 1px at 130px 80px, #fff, transparent),
        radial-gradient(2px 2px at 160px 30px, #eee, transparent);
    background-repeat: repeat;
    background-size: 200px 100px;
    animation: stars 120s linear infinite;
}

/* Мерцание звезд */
.twinkling {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
    background: transparent;
    animation: twinkling 8s ease-in-out infinite;
}

/* Облака */
.clouds {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
    height: 100%;
}

.cloud {
    position: absolute;
    background: rgba(255, 255, 255, 0.1);
    border-radius: 1000px;
    filter: blur(20px);
}

.cloud-1 {
    width: 300px;
    height: 100px;
    top: 20%;
    left: -100px;
    animation: cloudMove 25s linear infinite;
}

.cloud-2 {
    width: 400px;
    height: 120px;
    top: 40%;
    right: -100px;
    animation: cloudMove 30s linear infinite reverse;
}

.cloud-3 {
    width: 250px;
    height: 80px;
    top: 70%;
    left: -100px;
    animation: cloudMove 35s linear infinite;
}

.cloud-4 {
    width: 350px;
    height: 110px;
    top: 85%;
    right: -100px;
    animation: cloudMove 40s linear infinite reverse;
}

/* Анимации */
@keyframes stars {
    from { transform: translateX(0); }
    to { transform: translateX(-200px); }
}

@keyframes twinkling {
    0%, 100% { opacity: 0.3; }
    50% { opacity: 0.8; }
}

@keyframes cloudMove {
    from { transform: translateX(0); }
    to { transform: translateX(calc(100vw + 200px)); }
}

/* Основной контент */
.container {
    max-width: 800px;
    margin: 40px auto;
    background: rgba(255, 255, 255, 0.95);
    border-radius: 20px;
    box-shadow: 0 20px 40px rgba(0, 0, 0, 0.3);
    overflow: hidden;
    backdrop-filter: blur(10px);
    border: 1px solid rgba(255, 255, 255, 0.2);
    position: relative;
    z-index: 1;
}

header {
    background: linear-gradient(135deg, #005bff 0%, #00d4ff 100%);
    color: white;
    padding: 40px 30px;
    text-align: center;
    position: relative;
    overflow: hidden;
}

header:before {
    content: '';
    position: absolute;
    top: -50%;
    left: -50%;
    width: 200%;
    height: 200%;
    background: radial-gradient(circle, rgba(255,255,255,0.1) 1px, transparent 1px);
    background-size: 20px 20px;
    animation: headerStars 20s linear infinite;
}

@keyframes headerStars {
    from { transform: translateX(0) translateY(0); }
    to { transform: translateX(-20px) translateY(-20px); }
}

header h1 {
    font-size: 2.5em;
    margin-bottom: 10px;
    text-shadow: 0 2px 10px rgba(0, 0, 0, 0.3);
    position: relative;
    z-index: 1;
}

header p {
    font-size: 1.1em;
    opacity: 0.9;
    position: relative;
    z-index: 1;
}

.search-section, .barcode-section {
    padding: 40px 30px;
    border-bottom: 1px solid rgba(0, 0, 0, 0.1);
    position: relative;
}

.search-section:last-child, .barcode-section:last-child {
    border-bottom: none;
}

h2 {
    color: #333;
    margin-bottom: 25px;
    font-size: 1.5em;
    display: flex;
    align-items: center;
    gap: 10px;
}

.search-box, .barcode-controls {
    display: flex;
    gap: 15px;
    margin-bottom: 25px;
    flex-wrap: wrap;
}

input[type="text"], select {
    flex: 1;
    padding: 15px 20px;
    border: 2px solid rgba(0, 91, 255, 0.2);
    border-radius: 12px;
    font-size: 16px;
    transition: all 0.3s;
    min-width: 200px;
    background: rgba(255, 255, 255, 0.9);
}

input[type="text"]:focus, select:focus {
    outline: none;
    border-color: #005bff;
    box-shadow: 0 0 0 3px rgba(0, 91, 255, 0.1);
    transform: translateY(-2px);
}

button {
    padding: 15px 30px;
    background: linear-gradient(135deg, #005bff 0%, #00d4ff 100%);
    color: white;
    border: none;
    border-radius: 12px;
    font-size: 16px;
    cursor: pointer;
    transition: all 0.3s;
    font-weight: 600;
}

button:hover {
    transform: translateY(-3px);
    box-shadow: 0 10px 20px rgba(0, 91, 255, 0.3);
}

button:active {
    transform: translateY(-1px);
}

.results {
    margin-top: 25px;
    padding: 20px;
    border-radius: 12px;
    background: rgba(248, 249, 250, 0.8);
    min-height: 60px;
    backdrop-filter: blur(5px);
}

.product-card {
    background: white;
    padding: 20px;
    border-radius: 12px;
    box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
    margin-bottom: 15px;
    border-left: 4px solid #005bff;
    transition: transform 0.3s;
}

.product-card:hover {
    transform: translateX(5px);
}

.product-card h3 {
    color: #005bff;
    margin-bottom: 10px;
    font-size: 1.3em;
}

.barcode-output {
    text-align: center;
    margin-top: 25px;
    padding: 25px;
    background: rgba(248, 249, 250, 0.8);
    border-radius: 12px;
    backdrop-filter: blur(5px);
}

#barcode {
    max-width: 100%;
    height: auto;
    background: white;
    padding: 20px;
    border-radius: 10px;
    box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
}

.barcode-text {
    margin-top: 15px;
    font-family: monospace;
    font-size: 18px;
    font-weight: bold;
    color: #333;
}

.error {
    color: #ff4444;
    background: rgba(255, 234, 234, 0.9);
    padding: 15px;
    border-radius: 8px;
    margin-top: 15px;
    border-left: 4px solid #ff4444;
}

.success {
    color: #00c851;
    background: rgba(232, 245, 232, 0.9);
    padding: 15px;
    border-radius: 8px;
    margin-top: 15px;
    border-left: 4px solid #00c851;
}

/* Анимация появления элементов */
.container > * {
    animation: fadeInUp 0.6s ease-out;
}

@keyframes fadeInUp {
    from {
        opacity: 0;
        transform: translateY(30px);
    }
    to {
        opacity: 1;
        transform: translateY(0);
    }
}

@media (max-width: 600px) {
    .container {
        margin: 20px;
        border-radius: 15px;
    }
    
    .search-box, .barcode-controls {
        flex-direction: column;
    }
    
    input[type="text"], select, button {
        width: 100%;
    }
    
    header h1 {
        font-size: 2em;
    }
    
    .cloud {
        display: none; /* Скрываем облака на мобильных */
    }
}
